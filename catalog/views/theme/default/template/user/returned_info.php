		...
		<div id="middle" class="<?php echo $class; ?> middle-flat-left user-order-list">
			...
			<table class="table" style="margin: 10px 0;border:1px solid rgb(217, 217, 217);">
				...
				<tbody>
					...
					<tr>
						<td class="col-md-1 col-sm-1 col-xs-2 text-left border-right"><a target="_blank" href="<?php echo $this->config->item('catalog').'product?product_id='.$return_info['product_id'];?>"><img width="<?php echo $this->config->get_config('wish_cart_image_size_b_w');?>px" height="<?php echo $this->config->get_config('wish_cart_image_size_b_h');?>px" class="media-object lazy" data-original="<?php echo $this->image_common->resize($return_info['image'], $this->config->get_config('wish_cart_image_size_b_w'), $this->config->get_config('wish_cart_image_size_b_h'));?>" alt="<?php echo $return_info['name']; ?>"></a></td>
						<td class="text-left col-md-3 col-sm-3 col-xs-4 border-right"><span><a target="_blank" href="<?php echo $this->config->item('catalog').'product?product_id='.$return_info['product_id'];?>"><?php echo $return_info['name']; ?></a></span><span class="value"><?php echo !empty($return_info['option']) ? $return_info['option'] : '';?></span></td>
			...
			<?php if($return_info['return_history'][0]['return_status_id'] == $this->config->get_config('merchant_refused')):?>
			退款处理
			<hr>
			<form action="<?php echo $this->config->item('catalog').'user/returned/action?token='.$_SESSION['token'].'&rowid='.$this->input->get('rowid');?>" method="post" enctype="multipart/form-data" class="form-horizontal">
			...
			<div class="form-group">
				<div class="col-sm-6 text-right"><button type="submit" class="btn btn-warning btn-sm" id="refuse">撤销退款</button></div>
				<div class="col-sm-6"><button type="submit" class="btn btn-success btn-sm" id="agree">申请平台介入</button></div>
				...
				<input type="hidden" name="user_id" value="<?php echo $return_info['s_user_id'];?>">
			</div>
          </form>
          <script type="text/javascript">
			...
			<?php if($return_info['return_history'][0]['return_status_id'] == $this->config->get_config('wait_returns')):?>
			退款处理
			<hr>
			<form action="<?php echo $this->config->item('catalog').'user/returned/action?token='.$_SESSION['token'].'&rowid='.$this->input->get('rowid');?>" method="post" enctype="multipart/form-data" class="form-horizontal">
			<div class="form-group">
          ...
          <script type="text/javascript">
			...
			<?php if($return_info['return_history'][0]['return_status_id'] == $this->config->get_config('waiting_c_payments')):?>
			退款处理
			<hr>
			<form action="<?php echo $this->config->item('catalog').'user/returned/c_action?token='.$_SESSION['token'].'&rowid='.$this->input->get('rowid');?>" method="post" enctype="multipart/form-data" class="form-horizontal">
			<div class="form-group">
				<div class="col-sm-12" for="comment">退款说明</div>
				<div class="col-sm-12">
					<textarea rows="5" id="comment" name="comment" class="form-control"></textarea>
			...
			<div class="form-group">
				<div class="col-sm-6 text-right"><button type="submit" class="btn btn-warning btn-sm" id="refuse">未收到款，平台介入</button></div>
				<div class="col-sm-6"><button type="submit" class="btn btn-success btn-sm" id="agree">确认收款</button></div>
				<input type="hidden" name="return_id" value="<?php echo $return_info['return_id'];?>">
				<input type="hidden" name="action" id="action" value="">
				<input type="hidden" name="user_id" value="<?php echo $return_info['s_user_id'];?>">
			</div>
