...
$('#button-search').on('click', function(e) {
	var url = '<?php echo $this->config->item('admin').'common/filemanager?directory='.$directory;?>';
...
$('#button-upload').on('click', function() {
	...
	timer = setInterval(function() {
		if ($('#form-upload input[name=\'file\']').val() != '') {
			...
			$.ajax({
				url: '<?php echo $this->config->item('admin').'common/filemanager/upload?directory='.$directory;?>',
				type: 'post',		
				dataType: 'json',
...
$('#button-folder').on('shown.bs.popover', function() {
	$('#button-create').on('click', function() {
		$.ajax({
			url: '<?php echo $this->config->item('admin').'common/filemanager/folder?directory='.$directory;?>',
			type: 'post',		
			dataType: 'json',
...
$('#modal-image #button-delete').on('click', function(e) {
	if (confirm("<?php echo lang_line('text_confirm');?>")) {
		$.ajax({
			url: '<?php echo $this->config->item('admin').'common/filemanager/delete';?>',
			type: 'post',		
			dataType: 'json',
