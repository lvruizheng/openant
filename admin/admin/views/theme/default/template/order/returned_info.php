		...
		<div id="middle" class="<?php echo $class; ?> middle-flat-left user-order-list">
			...
			<table class="table" style="margin: 10px 0;border:1px solid rgb(217, 217, 217);">
				...
				<tbody>
					...
					<tr>
						<td class="col-md-1 col-sm-1 col-xs-2 text-left border-right"><a><img width="<?php echo $this->config->get_config('wish_cart_image_size_b_w');?>px" height="<?php echo $this->config->get_config('wish_cart_image_size_b_h');?>px" class="media-object lazy" data-original="<?php echo $this->image_common->resize($return_info['image'], $this->config->get_config('wish_cart_image_size_b_w'), $this->config->get_config('wish_cart_image_size_b_h'));?>" alt="<?php echo $return_info['name']; ?>"></a></td>
						<td class="text-left col-md-3 col-sm-3 col-xs-4 border-right"><span><a><?php echo $return_info['name']; ?></a></span><span class="value"><?php echo !empty($return_info['option']) ? $return_info['option'] : '';?></span></td>
			...
			<div class="form-group">
				<div class="col-sm-6 text-right"><button type="submit" class="btn btn-warning btn-sm" id="refuse">平台拒绝退款</button></div>
				<div class="col-sm-6"><button type="submit" class="btn btn-success btn-sm" id="agree">平台同意退款</button></div>
				...
				<input type="hidden" name="s_user_id" value="<?php echo $return_info['s_user_id'];?>">
				<input type="hidden" name="o_user_id" value="<?php echo $return_info['o_user_id'];?>">
			</div>
          </form>
