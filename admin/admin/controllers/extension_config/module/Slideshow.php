...
class Slideshow extends MY_Controller {
	...
	public function edit(){
		...
		if($_SERVER['REQUEST_METHOD']=="POST" && $this->check_modify()){
			...
			redirect($this->config->item('admin').'extension_config/module/slideshow');
		}
	...
	{
		if($this->input->get('module_id') != NULL){
			$data['action']=$this->config->item('admin').'extension_config/module/slideshow/edit?module_id='.$this->input->get('module_id');
			//查数据
			$module=$this->module_model->get_modules_for_module_id($this->input->get('module_id'));
			//var_dump($data['module']);
			
		}else{
			$data['action']=$this->config->item('admin').'extension_config/module/slideshow/add';
		}
