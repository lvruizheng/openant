...
class Setting extends MY_Controller
{
	...
	public function get_form()
	{
		...
		foreach($settings as $key=>$value)
		{
			...
			//退换商品事件
			if($this->input->post('request_refund') != NULL && !isset($request_refund))
			{
				$data['request_refund'] = $this->input->post('request_refund');
				$request_refund = TRUE;
			}
			elseif($settings[$key]['key'] == 'request_refund' && !isset($request_refund))
			{
				$data['request_refund'] = $settings[$key]['value'];
				$request_refund = TRUE;
			}
			
			if($this->input->post('merchant_confirmation') != NULL && !isset($merchant_confirmation))
			{
				$data['merchant_confirmation'] = $this->input->post('merchant_confirmation');
				$merchant_confirmation = TRUE;
			}
			elseif($settings[$key]['key'] == 'merchant_confirmation' && !isset($merchant_confirmation))
			{
				$data['merchant_confirmation'] = $settings[$key]['value'];
				$merchant_confirmation = TRUE;
			}
			
			if($this->input->post('merchant_refused') != NULL && !isset($merchant_refused))
			{
				$data['merchant_refused'] = $this->input->post('merchant_refused');
				$merchant_refused = TRUE;
			}
			elseif($settings[$key]['key'] == 'merchant_refused' && !isset($merchant_refused))
			{
				$data['merchant_refused'] = $settings[$key]['value'];
				$merchant_refused = TRUE;
			}
			
			if($this->input->post('wait_returns') != NULL && !isset($wait_returns))
			{
				$data['wait_returns'] = $this->input->post('wait_returns');
				$wait_returns = TRUE;
			}
			elseif($settings[$key]['key'] == 'wait_returns' && !isset($wait_returns))
			{
				$data['wait_returns'] = $settings[$key]['value'];
				$wait_returns = TRUE;
			}
			
			if($this->input->post('returns_goods') != NULL && !isset($returns_goods))
			{
				$data['returns_goods'] = $this->input->post('returns_goods');
				$returns_goods = TRUE;
			}
			elseif($settings[$key]['key'] == 'returns_goods' && !isset($returns_goods))
			{
				$data['returns_goods'] = $settings[$key]['value'];
				$returns_goods = TRUE;
			}
			
			if($this->input->post('wait_m_returns') != NULL && !isset($wait_m_returns))
			{
				$data['wait_m_returns'] = $this->input->post('wait_m_returns');
				$wait_m_returns = TRUE;
			}
			elseif($settings[$key]['key'] == 'wait_m_returns' && !isset($wait_m_returns))
			{
				$data['wait_m_returns'] = $settings[$key]['value'];
				$wait_m_returns = TRUE;
			}
			
			if($this->input->post('waiting_c_payments') != NULL && !isset($waiting_c_payments))
			{
				$data['waiting_c_payments'] = $this->input->post('waiting_c_payments');
				$waiting_c_payments = TRUE;
			}
			elseif($settings[$key]['key'] == 'waiting_c_payments' && !isset($waiting_c_payments))
			{
				$data['waiting_c_payments'] = $settings[$key]['value'];
				$waiting_c_payments = TRUE;
			}
			
			if($this->input->post('returns_goods_success') != NULL && !isset($returns_goods_success))
			{
				$data['returns_goods_success'] = $this->input->post('returns_goods_success');
				$returns_goods_success = TRUE;
			}
			elseif($settings[$key]['key'] == 'returns_goods_success' && !isset($returns_goods_success))
			{
				$data['returns_goods_success'] = $settings[$key]['value'];
				$returns_goods_success = TRUE;
			}
			
			if($this->input->post('intervention') != NULL && !isset($intervention))
			{
				$data['intervention'] = $this->input->post('intervention');
				$intervention = TRUE;
			}
			elseif($settings[$key]['key'] == 'intervention' && !isset($intervention))
			{
				$intervention = TRUE;
				$data['intervention'] = $settings[$key]['value'];
			}
			
			if($this->input->post('platform_refused') != NULL && !isset($platform_refused))
			{
				$data['platform_refused'] = $this->input->post('platform_refused');
				$platform_refused = TRUE;
			}
			elseif($settings[$key]['key'] == 'platform_refused' && !isset($platform_refused))
			{
				$platform_refused = TRUE;
				$data['platform_refused'] = $settings[$key]['value'];
			}
			
			if($this->input->post('cancellation_refund') != NULL && !isset($cancellation_refund))
			{
				$data['cancellation_refund'] = $this->input->post('cancellation_refund');
				$cancellation_refund = TRUE;
			}
			elseif($settings[$key]['key'] == 'cancellation_refund' && !isset($cancellation_refund))
			{
				$cancellation_refund = TRUE;
				$data['cancellation_refund'] = $settings[$key]['value'];
			}
		...
		$data['return_statuss'] = $this->return_status_model->get_return_statuss_for_setting();
