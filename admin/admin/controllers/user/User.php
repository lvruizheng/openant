...
class User extends MY_Controller {
	...
	public function __construct() {
		...
		$this->load->model(array('common/user_model', 'user/user_reward_model', 'common/user_class_model', 'localisation/address_model','localisation/location_model', 'common/competence_model'));
	}
	...
	public function get_user_rewards($in=''){
		if($this->input->get('user_id')){
			...
			$data['user_id']				=$this->input->get('user_id');
			$user_rewards					=$this->user_reward_model->get_user_rewards($data);
	...
	public function add_reward(){
		if($this->input->post('points') != NULL && $this->input->get('user_id') != NULL){
			...
			if($this->check_modify() && $this->form_validation->numeric($this->input->get('user_id')) && $this->form_validation->numeric($this->input->post('points')) && $this->form_validation->max_length($this->input->post('description'),'2000')){
				$data['description'] = $this->input->post('description');
				$data['user_id'] = $this->input->get('user_id');
				$data['points'] = (int)$this->input->post('points');
				$data['event'] = 'admin_add';
				
				$this->user_reward_model->add_reward($data);
			}
		}else{
			return FALSE;
		}
